...
#   PANEL_URL, API_TOKEN, SETTINGS_UUID, HEALTH_SHORT_UUID, PROFILE
...
api PATCH /api/subscription-settings \
  -d "$(jq -n --arg u "$SETTINGS_UUID" --arg r "$LINK" '{uuid:$u, happRouting:$r}')" \
  | jq -e '.response != null' >/dev/null || die "PATCH happRouting не прошёл"
...
if [ -n "${HEALTH_SHORT_UUID:-}" ]; then
  ...
  if [ "$CODE" != 200 ]; then
    api PATCH /api/subscription-settings \
      -d "$(jq -n --arg u "$SETTINGS_UUID" --arg r "$PREV" \
            '{uuid:$u, happRouting:(if $r=="" then null else $r end)}')" >/dev/null
...
log "OK залит профиль $PROFILE (${#LINK} символов)"
